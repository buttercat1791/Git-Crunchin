...
class Redactor:
    ...
    ad_regexes: dict[str, Pattern[AnyStr@compile]] = {
        ...
        'pilgirmage_end': re.compile(r'(?i)now[.,;]?\s+back\s+to\s+the\s+episode\s+[,;]?guys[.,;]?\s+yeah[.]?\s+'),
    }
    ...
    def remove_ads(self, transcript: str) -> str:
        ...
        for ad in self.ad_list:
            start_match: Match[str] = self.ad_regexes[ad[0]].search(transcript)
            end_match: Match[str] = self.ad_regexes[ad[1]].search(transcript)
